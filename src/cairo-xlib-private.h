...
struct _cairo_xlib_display {
    ...
    xuint32_t solid_cache[32]; /* low 16 are opaque, high 16 transparent */
    struct {
	xuint32_t color;
	int index;
    } last_solid_cache[2];
...
typedef struct _cairo_xlib_visual_info {
    cairo_list_t link;
    VisualID visualid;
    struct { xuint8_t a, r, g, b; } colors[256];
    xuint8_t cube_to_pseudocolor[CUBE_SIZE][CUBE_SIZE][CUBE_SIZE];
    xuint8_t field8_to_cube[256];
    xint8_t  dither8_to_cube[256];
    xuint8_t gray8_to_pseudocolor[256];
...
struct _cairo_xlib_screen {
    ...
    GC gc[GC_CACHE_SIZE];
    xuint8_t gc_depths[GC_CACHE_SIZE];
